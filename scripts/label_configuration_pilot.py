#!/usr/bin/env python3
"""Run and evaluate the label-configuration pilot without changing the input Excel.
...
def args():
    ...
    p.add_argument("--output-dir", type=Path, default=SCRIPT_DIR / "outputs" / "label_configuration_pilot")
    p.add_argument("--evaluate-only", action="store_true", help="Evaluate existing pilot workbooks only; no model/API calls.")
    return p.parse_args()
...
def run_one(a, dataset, base_prompt, config, repetition):
    ...
    path=outdir/f"label_pilot_{config}_{a.model}_run{repetition:02d}_{stamp}.xlsx"
    result=dataset.copy()
    for col in ("Predicted_Label","Is_Ambiguous","Alternative_Labels","Is_Not_Decidable","Confidence_Percent","Reasoning","Raw_Model_Response","JSON_Parse_Status","Processing_Status","Files_Used","File_Count","Run_Model","Label_Config","Pilot_Repetition","Run_Timestamp"): result[col]=pd.NA
    ...
    for i,row in result.iterrows():
        result.loc[i,["Run_Model","Label_Config","Pilot_Repetition","Run_Timestamp"]]=[a.model,config,repetition,datetime.now().isoformat(timespec="seconds")]
        try:
...
def evaluate(a):
    ...
    if not books: raise FileNotFoundError(f"No pilot workbooks found below {a.output_dir}")
    allcases=[]
    for p in books:
    ...
    cases=pd.concat(allcases,ignore_index=True); report=a.output_dir/"Label_Configuration_Pilot_Evaluation.xlsx"
