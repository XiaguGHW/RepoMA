...
LOG_PATH = LOG_DIR / f"gemini_P1_P2_first20_test_{RUN_TIMESTAMP}.log"
RESULT_PATH = RESULTS_DIR / f"gemini_P1_P2_first20_test_{RUN_TIMESTAMP}.xlsx"
RAW_RESPONSES_PATH = (
    RESPONSES_DIR / f"gemini_P1_P2_first20_responses_{RUN_TIMESTAMP}.jsonl"
)
...
def select_priority_1_and_2_files(
    inventory_df: pd.DataFrame,
    folder_name: str,
) -> tuple[list[Path], list[Path], list[dict[str, str]]]:
    """Waehlt alle P1- und P2-Dateien des gematchten Baugruppenordners aus."""
    folder_mask = inventory_df[INVENTORY_FOLDER_COL].map(clean_cell).eq(folder_name)
    normalized_priorities = {value.casefold() for value in PRIORITY_VALUES}
    priority_mask = (
        inventory_df[INVENTORY_PRIORITY_COL]
        .map(clean_cell)
        .str.casefold()
        .isin(normalized_priorities)
    )
        ...
        file_details.append(
            {
                "Folder": folder_name,
                "Filename": clean_cell(inventory_row[INVENTORY_FILENAME_COL]),
                "Input_Priority": clean_cell(
                    inventory_row[INVENTORY_PRIORITY_COL]
                ),
...
def prepare_test_cases(
    ...
    for position, (source_index, row) in enumerate(test_df.iterrows(), start=1):
        ...
        existing_files, missing_files, file_details = select_priority_1_and_2_files(
            inventory_df,
            matched_folder,
        )
        ...
        case["Priority_1_2_Count"] = len(existing_files) + len(missing_files)
        case["Priority_1_2_Files"] = "\n".join(
        ...
        if not existing_files and not missing_files:
            case["Status"] = "NO_PRIORITY_1_2_FILES"
            case["Error_Message"] = (
                f"Keine Zeile mit {INVENTORY_PRIORITY_COL} in "
                f"{PRIORITY_VALUES} gefunden"
            )
        elif missing_files:
            case["Status"] = "FILE_NOT_FOUND"
            case["Error_Message"] = (
                "Mindestens eine Priority-1/2-Datei fehlt; API-Aufruf wird "
...
def run_api_for_ready_cases(
    ...
    for api_position, case in enumerate(ready_cases, start=1):
        ...
        logging.info(
            "Gemini-Aufruf %s/%s: ID=%s, Name=%s, P1/P2-Dateien=%s",
            api_position,
            len(ready_cases),
            case["ID"],
            case["Benennung (EN)"],
...
def main() -> None:
    ...
    for case in cases:
        logging.info(
            "Precheck-Ergebnis: ID=%s | Match=%s | Ordner=%s | P1/P2=%s | Status=%s",
            case["ID"],
            case["Match_Method"],
            case["Matched_Folder"],
            case["Priority_1_2_Count"],
            case["Status"],
        )
